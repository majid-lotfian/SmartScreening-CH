...
OUT_DIR    = "new_results/stage2_cost_sensitive-SHAP"
...
print("\n🔍 Generating SHAP explanation for final CatBoost model...")

# Use TreeExplainer for CatBoost
#explainer = shap.TreeExplainer(final_model.base_estimator)  # extract CatBoost from CalibratedClassifierCV

# Get the fitted CatBoost model
fitted_catboost = final_model.estimator

# Create SHAP explainer
explainer = shap.TreeExplainer(fitted_catboost)


# Use a small number of rows for speed (you can increase this)
#X_explain = X_all.sample(n=50, random_state=123)

# use all CH members of test set for SHAP
df_test = load_csv(TEST_FILE)
df_test = df_test[df_test["CH3"] != 0].copy()
X_test = df_test.drop(columns=["CH3", "kind"]).fillna(df_all.median())

#X_explain = X_test.sample(n=200, random_state=123)  # Use full test set if feasible
X_explain = X_test.copy()
shap_values = explainer.shap_values(X_explain)

# Plot summary plot
shap.summary_plot(shap_values, X_explain, show=False)
plt.title("SHAP Summary – CatBoost (CH-C vs CH-T)")
plt.tight_layout()
plt.savefig(os.path.join(OUT_DIR, "shap_summary_catboost.png"))
plt.close()

# Save SHAP values to CSV
shap_df = pd.DataFrame(shap_values, columns=X_explain.columns)
shap_df.insert(0, "PatientIndex", X_explain.index)
shap_df.to_csv(os.path.join(OUT_DIR, "shap_values_catboost.csv"), index=False)

# Optional: also save feature values
combined_df = pd.concat([X_explain.reset_index(drop=True), shap_df.drop(columns="PatientIndex")], axis=1)
combined_df.to_csv(os.path.join(OUT_DIR, "shap_inputs_and_values_catboost.csv"), index=False)

print("✅ SHAP outputs saved (plot + CSV).")
